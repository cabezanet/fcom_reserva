$(function(e){
	...
	function onLoad(){
		...
		$('#btnNuevaReserva').click(function(e){
			if ($('#alert_msg').data('nh') > 12){
				...
				$('#editOptions').hide();
				resetMsgErrors();
				//$('#datepickerFinicio').val(strDateCurrentMonth($('#datepicker').val()));
	...
	function whenChangeRecurseSelected(){
		...
		$('#selectGroupRecurse').on('change',function(e){
			...
			if ($viewActive != 'agenda'){
				showGifEspera();
				$('#message').fadeOut("slow");
				console.log($('form#selectRecurse').serialize());
				$.ajax({
					type:"GET",
					url:"ajaxGetRecursoByGroup",
					data: $('form#selectRecurse').serialize(),
	...
	function whenChangeInputInModalWindow(){
		...
		$( "#newReservaRepetir" ).on('change',function(){
			if ($('#newReservaRepetir').val() == 'CS') {
				...
				$('#datepickerFfin').val(nextMonth(dateToformatES($("#datepickerFinicio").val())));
			}
			else $('#inputRepeticion').slideUp('slow');
			setResumen();
		});
	...
	*/
	...
	function resetMsgErrors(){
		...
		var $labels = new Array('titulo','fInicio','hFin','fFin','dias');
		for(var key in $labels){
					//console.log($labels[key]);
					$('#'+$labels[key]).removeClass('has-error');
					//console.log($('#'+$labels[key]+'_Error'));
	...
	*/
	...
	function printCalendar(){
		...
		console.log('identificxador de grupo = ' + $grupo_id);
		console.log('identificxador de recurso = ' + $id_recurso);
		//if ($id_recurso != '' || $viewActive == 'agenda') {
			
			showGifEspera();
			$.ajax({
					type:"GET",
					url:"ajaxCalendar",
					data:{viewActive: $viewActive,day: $aDate[0],month: $aDate[1], year: $aDate[2],id_recurso: $id_recurso,grupo_id: $grupo_id},
					success: function(respuesta){
						...
						if ($viewActive == 'agenda') {
							...
							if ($id_recurso == '')	$('#alert').fadeIn();
							//$('#alert_evento').fadeOut();
						}
						hideGifEspera();
						},
						error: function(xhr, ajaxOptions, thrownError){
        	...
        	success: function(respuesta){
        		if (respuesta['error'] == false){
					...
					$('#actionType').val('');
 					...
 					//console.log(respuesta['idsSolapamientos']);
	...
	*/
	...
	function hasnewdata($idEvento){
		...
		if ($('select[name|="repetir"]').val() != $value) {
			$newdata = true;
			console.log('repetir');
			console.log($('select[name|="repetir"]').val());
			console.log($value);}
		// fecha inicio
		if ($value == 'CS'){
			if ($('#datepickerFevento').val() != dateToformatES($respuesta.fechaInicio)) {
				...
				console.log($('#datepickerFevento').val());
				console.log(dateToformatES($respuesta.fechaEvento));}
			//fechaEvento
			if ($('#datepickerFinicio').val() != dateToformatES($respuesta.fechaEvento)) {
				$newdata = true;
				console.log('fechaEvento');
				console.log($('#datepickerFinicio').val());
				console.log(dateToformatES($respuesta.fechaInicio));}
		}
		else {
		...
		if ($('#datepickerFfin').val() != dateToformatES($respuesta.fechaFin)) {
			...
			console.log($('#datepickerFfin').val());
			console.log(dateToformatES($respuesta.fechaFin));}
		//días
	...
	function initModalEdit($idEvento,$idSerie){
		...
		//$('button#editOption2').off();
		//$('button#editOption3').off();
	...
	function setCheckBoxActive($fecha){
		...
		$("input:checkbox").each(function(index,value){
			...
			if (index === num) {
				console.log('active día index = ' + num);
	...
	function setLabelRecurseName(){
		$viewActive = $('#btnView .active').data('calendarView');
		if ($viewActive == 'agenda') $strCalendar = 'todos lo espacios o medios';
		else $strCalendar = $('select#recurse option:selected').text();
	...
	function firstDayAviable(){
		console.log($('.formlaunch').first().data('fecha'));
